                ...
                <div class="card-body">
                    ...
                    <?php else: ?>
                        <div class="space-y-3">
                            ...
                            $pending_bookings_today = array_filter($today_bookings, fn($b) => $b['status'] === 'pending');
                            $other_bookings = array_filter($today_bookings, fn($b) => !in_array($b['status'], ['approved', 'pending']));
                    ...
                    <div class="bg-white/20 rounded-lg p-4 backdrop-blur-sm">
                        ...
                        <div id="messagePreview" class="bg-black/30 rounded p-3 text-sm font-mono">
                            ...
                            <div class="mt-2">
                                ...
                                <div class="ml-2">
                                    <div>✅ อนุมัติแล้ว: <?php echo count(array_filter($today_bookings, fn($b) => $b['status'] === 'approved')); ?> รายการ</div>
                                    <div>⏳ รออนุมัติ: <?php echo count(array_filter($today_bookings, fn($b) => $b['status'] === 'pending')); ?> รายการ</div>
