...
// ล้าง buffer ที่อาจมี error หรือ warning
ob_clean();
...
// ล้าง buffer และตั้ง header
ob_clean();
header('Content-Type: application/json; charset=utf-8');

// Debug logging
error_log("=== TELEGRAM SEND DEBUG ===");
...
try {
    ...
    foreach ($recipients_list as $user) {
        try {
            // ตรวจสอบว่า user มีการตั้งค่า Telegram หรือไม่
            ...
            // ส่งข้อความโดยใช้ข้อมูล Telegram ของ user นั้นๆ
    ...
    ob_clean(); // ล้าง buffer ก่อน echo JSON
    ...
    ob_clean(); // ล้าง buffer ก่อน echo JSON
    error_log("TELEGRAM SEND FATAL ERROR: " . $t->getMessage());
    
    echo json_encode([
        'success' => false,
        'message' => 'เกิดข้อผิดพลาดร้ายแรง: ' . $t->getMessage(),
        'debug' => [
            'file' => $t->getFile(),
            'line' => $t->getLine()
        ]
